...
\begin{mydef}
Metrickým prostorem budeme rozumět dvojici $\mathcal{X} = (X, XXX)$, kde $X$ je množina, jejíž prvky nazýváme bodu a $XXX$ je tzv. vzdálenost (metrika), což je nezáporná reální funkce $XXX(x, y)$, která je definována pro každou dvojicí $x, y \in X$ (tedy $XXX: X \times X \to \mathbb{R}_0^+$ je zobrazení) a která splňuje tyto podmínky:
\begin{enumerate}[1)]
	\item $\forall x, y \in X: XXX(x, y) = 0 \Leftrightarrow x = y$
	\item $\forall x, y \in X: XXX(x, y) = XXX(y, x)$ (symetrie)
	\item $\forall x, y, z \in X: XXX(x, y) + XXX(y, z) \geq XXX(x, z)$ (trojúhelníková nerovnost)
\end{enumerate}
\end{mydef}

Příklady:
\begin{itemize}
	\item Diskrétní metrický prostor $(X, XXX)$:
	$$XXX(x, y) = \begin{dcases}
	0 \text{v případě} x = y\\
	1 \text{v případě} x \not= y
	\end{dcases}$$
	\item Metrický prostor $\mathbb{R}^1$ se vzdáleností $XXX(x, y) = |x-y|$
	\item $n$-rozměrný euklidovský prostor $\mathbb{R}^n$ se vzdáleností $XXX(x, y) = \sqrt{\sum\limits_{k = 1}^n}(y_k - x_k)^2$. Tento metrický prostor $(\mathbb{R}^n, XXX)$ značíme $\mathbb{R}^n_1$.
	\item Metrický prostor $\mathbb{R}^n_0$ používá vzdálenost $XXX_0(x, y) = \max\limits_{1 \leq k \leq n} |y_k - x_k|$
	\item Množina $C(\left<a, b \right>)$ (často též značená $C^0\left<a,b\right>$) všech spojitých reálných funkcí, které jsou definovány na intervalu $\left<a, b\right>$, se vzdáleností: $XXX(f, g) = \max\limits_{a \leq t \leq b} | g(t) - f(t)$ také tvoří metrický prostor.
	...
	$XXX(x, y) = \sqrt{\sum\limits_{k=1}^\infty(y_k - x_k)^2}$
	\item Uvažujeme množinu všech spojitých funkcí na intervalu $\left<a, b\right>$. Vzdálenost definujeme jako $XXX(x, y) = \left( \int_a^b (x(t) - y(t))^2 dt \right)^\frac{1}{2}$. Tento metrický prostor budeme značit $C_2^0\left<a, b\right>$ a nazývat prostorem spojitých funkcí s kvadratickou metrikou.
	\item Uvažujme množinu všech ohraničených posloupností $x = (x_1, x_2, \dots, x_n, \dots)$ reálných čísel. Položíme-li $XXX(x, y) = sup |x_k - y_k|$ dostaneme metrický prostor $M^\infty$.
\end{itemize}

\begin{mydef}
Nechť $\mathcal{X} = (X, XXX)$ je libovolný metrický prostor. Metrický prostor $\mathcal{M} = (M, XXX)$ s toutéž metrikou $XXX$ uvažovanou pouze na množině $M \subset X$ se nazývá podprostorem metrického prostoru $X$.
\end{mydef}

\section{Konvergence posloupností}
Buď $\mathcal{X} = (X, XXX)$ metrický prostor, $x_0 \in X, r \in \mathbb{R}^+$. \emph{Otevřenou koulí} $S(x_0, r)$ v metrickém prostoru $\mathcal{X}$ budeme nazývat množinu bodů $x \in X$, které vyhovují podmínce
...
\begin{itemize}
	...
	\item V metrickém prostorech $I_2$ je hustou množinou množina všech posloupností $\{x_n\}$. kde $x_n$ jsou racionální čísla, přičemž počet čísel $x_n$ různých od nuly je pouze konečný a pro různé posloupnost obecně různý.
\end{itemize}
...
\begin{mydef}
Nechť $\mathcal{X} = (X, XXX)$ a $\mathcal{Y} = (Y, XXX^*)$ jsou dva různé metrické prostory. Zobrazení $f: \mathcal{X} \to \mathcal{Y}$ prostoru $\mathcal{X}$ do prostoru $\mathcal{Y}$ se nazývá spojité v bodě $x_0 \in X$, jestliže k libovolnému $\epsilon > 0$ lze najít takové $\delta > 0$, že pro všechny body $x$ s vlastností $XXX(x, x_0) < \delta$ platí
$$XXX^*(f(x), f(x_0)) < \epsilon$$
\end{mydef}
...
\begin{mydef}
Říkáme, že vzájemně jednoznačné zobrazení $y = f(x)$ metrického prostoru $\mathcal{X} = (X, XXX)$ na metrický prostor $\mathcal{Y} = (Y, XXX^*)$ je izometrické, jestliže

$$ XXX(x_1, x_2) = XXX^*(f(x_1), f(x_2)) \quad \forall x_1, x_2 \in X $$
...
$$ \forall m,n \geq N(\epsilon) \quad XXX(x_m, x_n) < \epsilon $$
\end{mydef}

\begin{mydef}
Jestliže v metrickém prostoru $\mathcal{X}$ libovolná cauchyovská posloupnost konverguje (tj. existuje $x \in \mathcal{X}$ tak, že $XXX(x_n, x) \to 0$), potom nazýváme tento prostor \emph{úplný}.
\end{mydef}
...
\begin{mydef}
Nechť $\mathcal{X} = (X, XXX)$ je metrický prostor. Zobrazení $A$ prostoru $\mathcal{X}$ do prostoru $\mathcal{X}$ se nazývá \emph{kontraktivní} (neboli kontrakce), existuje-li takové číslo $\alpha < 1$, ž epro libovolné dva body $x, y \in X$ platí nerovnost
$$XXX(Ax, Ay) \leq \alpha XXX(x, y)$$
\end{mydef}
...
1. semestr, MAT, \texttt{Zaklady\_funkcionalni\_analyzy\_opr.pdf}, 7. -  ?. kapitola
...
Každý lineární prostor $X(x)$ konečné dimenze $n$ je izomorfní s euklidovským prostorem $\mathbb{R}^n$, a proto můžeme považovat prvky uvažovaného prostoru $X(n)$ za n-tice reálných čísel.

\begin{veta}
(Riesz) Nechť $X(n)$ je normovaný prostor konečné dimenze $n$. Aby posloupnost $\{x_\nu = ($.
\end{veta}
